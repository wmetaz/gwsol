...
class LFGAAResNet101(torch.nn.Module):
    def __init__(self, k):
        ...
        self.conv_6 = torch.nn.Sequential(
            torch.nn.Conv2d(1024, self.k, kernel_size=3, stride=1, padding=1),
        ...
        self.fam1 = FAM(1024, 512, [28, 28], [14, 14], [8, 8], [4, 4])
        self.feat_agg1 = FeatAgg(512, 512)

        self.fam2 = FAM(512, 256, [56, 56], [28, 28], [14, 14], [8, 8])
        self.feat_agg2 = FeatAgg(256, 256)

        self.fam3 = FAM(256, 64, [112, 112], [56, 56], [28, 28], [14, 14])
        self.feat_agg3 = FeatAgg(64, 64)

        self.fam4 = FAM(64, 64, [112, 112], [56, 56], [28, 28], [14, 14])
        self.feat_agg4 = FeatAgg(64, 64)

        self.fam5 = FAM(64, 256, [56, 56], [28, 28], [14, 14], [8, 8])
    ...
    def forward(self, x):
        conv2_2 = self.conv1_layer(x)
        conv3_4 = self.conv2_layer(conv2_2)
        conv4_4 = self.conv3_layer(conv3_4)
        conv5_4 = self.conv4_layer(conv4_4)
