...
class ZeroShotLearningLoss(torch.nn.Module):
    ...
    def forward(self, x, attr_mask):
        ...
        '''
        # written by wyzeng
        another = torch.nn.functional.softmax(self.all_attr, dim=1)
        cor = torch.matmul(another, another.transpose(0, 1))
        inx = cor.argmax(dim=1)
        count = 0.0
        ...
        for i in range(len(self.all_attr)):
            cnt = 0.0
            for j in range(len(self.all_attr[0])):
                if self.all_attr[i ,j] == 0.0:
                    cnt = cnt + 1
